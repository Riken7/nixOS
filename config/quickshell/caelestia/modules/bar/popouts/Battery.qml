...
StyledText {
    text: {
        ...
        // Determine the state text based on the state value
        if (state == 1) {
            stateText = qsTr("Charging");
        }
        else if (state == 2) {
            stateText = qsTr("Discharging");
        }
        else if (state == 4) {
            stateText = qsTr("Full");
        }
        else {
            stateText = qsTr("Unknown");
        }

        return qsTr("%1: %2%").arg(stateText).arg(percentage);
    }
}
